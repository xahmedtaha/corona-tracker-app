let countriesSource = `AFGHANISTAN;AF
ÅLAND ISLANDS;AX
ALBANIA;AL
ALGERIA;DZ
AMERICAN SAMOA;AS
...
countriesSource
    .split('\n')
    .forEach((line) => {
        console.log('#lsdfldfk line: ', line);
        let row = line.split(';');

        console.log('#lsdfldfk row: ', row);
        if (row.length !== 2) {
            console.error('#lsdfdkfs invalid row: ', row, line);
            return;
        }
        ...
        let name = '';
        nameParts.split(/,\s+/).forEach((namePart) => {
            name += namePart;
 ...
 *
 *   // Returns 'CH'
 *   countrynames.getCountryCode('Switzerland')
 *   // Returns 'BB'
 *   countrynames.getCountryCode('BarbaDOS')
 *
...
exports.getCountryCode = (name) => fromName[name.toUpperCase()];
 ...
 *
 *   // Returns 'TONGA'
 *   countrynames.getCountryCode('TO')
 *   // Returns 'RÉUNION'
 *   countrynames.getCountryCode('re')
 *
...
exports.getCountryName = (code) => fromCode[code.toUpperCase()];
...
exports.getAllCodes = () => Object.keys(fromCode).slice(1).sort();
